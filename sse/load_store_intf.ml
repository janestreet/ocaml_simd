...
module type Load_store = sig @@ portable
  ...
  module Float_iarray : sig
    ...
    (** Load two floats from a [float iarray] at an arbitrary (unaligned) index.
        ...
        @raise Invalid_argument if [idx..idx+1] fails bounds checking. *)
    val get : float iarray @ local -> idx:int -> t

    (** Load two floats from a [float iarray] at an arbitrary (unaligned) index. Does not
        validate safety. *)
    val unsafe_get : float iarray @ local -> idx:int -> t
  end
  ...
  module Immediate_array : sig
    ...
    (** Load two immediates from an array at an arbitrary (unaligned) index. The returned
        ...
        @raise Invalid_argument if [idx..idx+1] fails bounds checking. *)
    val get_tagged : ('a : immediate64). 'a array @ local -> idx:int -> t

    (** Load two immediates from an array at an arbitrary (unaligned) index. The returned
        vector contains two _tagged_ 64-bit values. Does not validate safety. *)
    val unsafe_get_tagged : ('a : immediate64). 'a array @ local -> idx:int -> t

    (** Load two immediates from an array at an arbitrary (unaligned) index. The returned
        vector contains two _untagged_ 63-bit values.

        @raise Invalid_argument if [idx..idx+1] fails bounds checking. *)
    val get_and_untag : ('a : immediate64). 'a array @ local -> idx:int -> t

    (** Load two immediates from an array at an arbitrary (unaligned) index. The returned
        vector contains two _untagged_ 63-bit values. Does not validate safety. *)
    val unsafe_get_and_untag : ('a : immediate64). 'a array @ local -> idx:int -> t

    (** Store two immediates to an array at an arbitrary (unaligned) index. The given
        vector must contain two _untagged_ 63-bit values.

        @raise Invalid_argument if [idx..idx+1] fails bounds checking.
        @raise Invalid_argument if either int64's top bit is set. *)
    val tag_and_set : ('a : immediate64). 'a array @ local -> idx:int -> t -> unit

    (** Store two immediates to an array at an arbitrary (unaligned) index. The given
        vector must contain two _tagged_ 64-bit values.

        @raise Invalid_argument if [idx..idx+1] fails bounds checking.
        @raise Invalid_argument if either int64's bottom bit is not set. *)
    val set_tagged : ('a : immediate64). 'a array @ local -> idx:int -> t -> unit

    (** Store two immediates to an array at an arbitrary (unaligned) index. The given
        vector must contain two _untagged_ 63-bit values. Does not validate safety. *)
    val unsafe_tag_and_set : ('a : immediate64). 'a array @ local -> idx:int -> t -> unit

    (** Store two immediates to an array at an arbitrary (unaligned) index. The given
        vector must contain two _tagged_ 64-bit values. Does not validate safety. *)
    val unsafe_set_tagged : ('a : immediate64). 'a array @ local -> idx:int -> t -> unit
  end
  ...
  module Immediate_iarray : sig
    ...
    (** Load two immediates from an iarray at an arbitrary (unaligned) index. The returned
        ...
        @raise Invalid_argument if [idx..idx+1] fails bounds checking. *)
    val get_tagged : ('a : immediate64). 'a iarray @ local -> idx:int -> t

    (** Load two immediates from an iarray at an arbitrary (unaligned) index. The returned
        vector contains two _tagged_ 64-bit values. Does not validate safety. *)
    val unsafe_get_tagged : ('a : immediate64). 'a iarray @ local -> idx:int -> t

    (** Load two immediates from an iarray at an arbitrary (unaligned) index. The returned
        vector contains two _untagged_ 63-bit values.

        @raise Invalid_argument if [idx..idx+1] fails bounds checking. *)
    val get_and_untag : ('a : immediate64). 'a iarray @ local -> idx:int -> t

    (** Load two immediates from an iarray at an arbitrary (unaligned) index. The returned
        vector contains two _untagged_ 63-bit values. Does not validate safety. *)
    val unsafe_get_and_untag : ('a : immediate64). 'a iarray @ local -> idx:int -> t
  end
  ...
  module Nativeint_u_array : sig
    ...
    (** Load two int64s from a [nativeint# array] at an arbitrary (unaligned) index.
        ...
        @raise Invalid_argument if [idx..idx+1] fails bounds checking. *)
    val get : nativeint# array @ local -> idx:int -> t

    (** Load two int64s from a [nativeint# array] at an arbitrary (unaligned) index. Does
        not validate safety. *)
    val unsafe_get : nativeint# array @ local -> idx:int -> t

    (** Store two int64s to a [nativeint# array] at an arbitrary (unaligned) index.

        @raise Invalid_argument if [idx..idx+1] fails bounds checking. *)
    val set : nativeint# array @ local -> idx:int -> t -> unit

    (** Store two int64s to a [nativeint# array] at an arbitrary (unaligned) index. Does
        not validate safety. *)
    val unsafe_set : nativeint# array @ local -> idx:int -> t -> unit
  end
