...
(** {v
 Refer to the Intel Intrinsics Guide (SSE4.2) for detailed specifications.
    ...
    The comparison operation is specified by a ppx_simd directive: [%bstr] for
    byte strings, and [%wstr] for word strings.
    ...
    [Byte.cmpistri [%bstr Signed, Eq_each, Masked_neg, Least_sig] ~a ~b]
    ...
    Byte.cmpistrm [%bstr Signed, Eq_any, Pos, Vec_mask] ~a ~b
    ]
    ...
    Byte.cmpistrc [%bstr Signed, In_range, Masked_neg] ~a ~b
    ]

    Finding the index of the first byte of the first occurance of a in b:

    [Byte.cmpistri [%bstr Signed, Eq_ordered, Pos, Least_sig] ~a ~b]
    v} *)
...
module Byte : sig
  ...
  (** Specify comparison with pxp_simd: [%bstr S, C, P, M] where (S : Signed.t), (C :
  ...
  (** Specify comparison with pxp_simd: [%bstr S, C, P] where (S : Signed.t), (C :
  ...
  (** Specify comparison with pxp_simd: [%bstr S, C, P, I] where (S : Signed.t), (C :
  ...
  val cmpestrs : a:t -> b:t -> a_len:int -> b_len:int -> int
  val cmpestrz : a:t -> b:t -> a_len:int -> b_len:int -> int
  ...
  val cmpistrs : a:t -> b:t -> int
  val cmpistrz : a:t -> b:t -> int
end
...
module Word : sig
  ...
  (** Specify comparison with pxp_simd: [%wstr S, C, P, M] where (S : Signed.t), (C :
      Comparison.t), (P : Polarity.t), (M : Mask.t) *)
  external cmpestrm
  ...
  (** Specify comparison with pxp_simd: [%wstr S, C, P] where (S : Signed.t), (C :
      Comparison.t), (P : Polarity.t) *)
  external cmpestra
  ...
  (** Specify comparison with pxp_simd: [%wstr S, C, P, I] where (S : Signed.t), (C :
      Comparison.t), (P : Polarity.t), (I : Index.t) *)
  external cmpestri
